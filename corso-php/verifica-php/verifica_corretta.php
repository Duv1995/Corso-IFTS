...
<body>
    ...
    //inizializzo l'array per scorrere i miei valori random
    ...
    //reinizializzo un altro ciclo che scorre tutti gli elementi dell'array
    for ($i = 0; $i < count($voti); $i++) {
        ...
        if ($voti[$i] < $voto_peggiore) {
            ...
            //ogni volta che trovo un nuovo minimo devo azzerare il conteggio di quelli gia trovati!
            $peggiori = 0;
        }
        if ($voti[$i] == $voto_peggiore) {
            $peggiori++; //in questo modo aggiungi +1 solo se non cambia il minimo di riferimento
        }
    ...
    //calcolo quanti sono in lista d'attesa (elite-posti disponibili)

    //percentuale lista d'attesa = numero lista d'attesa / elite *100
    if ($posti > $potenzialmente_ammessi) {
        //prima valuto il caso in cui tutti possono essere ammessi e quindi non c'è alcuna lista d'attesa
        $percentuale_in_lista_attesa = 0;
    } else {
        $percentuale_in_lista_attesa = ($potenzialmente_ammessi - $posti) / $potenzialmente_ammessi * 100;
    }

    echo "ci sono $posti posti e $potenzialmente_ammessi studenti che possono essere ammessi <br>";
    echo "la percentuale di studenti in lista d'attesa è del $percentuale_in_lista_attesa % <br>";
